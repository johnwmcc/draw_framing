## Local source repository D:\Documents\GitHub\draw_framing
...
# any purpose and without fee is hereby granted, under the terms of the MIT Licence
...
module JWM
	...
	puts "******************************"
	puts "Class DrawFraming v0.58 loaded"
  ...
  # Axis_lock 0, 1, 2 for red, green, blue; false = unlocked; true = lock
    ...
    # Default to undefined axis for long dimension of timber
    @@axis_lock = [0,0,0]
	...
	def onLButtonDown(flags, x, y, view)
      ...
      if( @ip1.valid? )
          ...
          if @ip.face
            ...
            # Calculate angle between face normal and X_AXIS in the rg plane - i.e., rotation
            #   about the Z_AXIS
            ...
            rotate3 = vec3.angle_between X_AXIS
            puts "rotate3 angle = " + rotate3.radians.to_s
            
            # Set up transform to rotate the cross-section by this amount around Z_AXIS
            @tf3 = Geom::Transformation.rotation [0,0,0], Z_AXIS, -rotate3
            
            # Calculate angle between face normal and its projection onto the rg plane
            rotate4 = f.normal.angle_between vec3
          ...
          # Set state of tool - first pick made, waiting to determine plane of cross section
						@state = 1
...
## ready for a pushpull operation to create the desired length
  ...
  # Set @tf as transformation from origin to first pick point
  @tf = @ip1.transformation
	###puts "Text set to " + text
	view.draw_text view.  screen_coords(pt2), @cursor_text # Give feedback at cursor
	if @plane != "no" #for orthogonal cases only r
		view.line_width = 2; view.line_stipple = ""
		@lAxis[(n/2).to_int] = d*2.0 
    # Set long axis direction normal to @plane and relocate its centre to first pick point
		@lAxis.transform!(@tf5)
    @lAxis.transform!(@tf)
		...
		# Have to rotate feedback polyline to be perpendicular to face normal at first pick point, 
    #   then move from World origin first to component origin, then add translation to pick point

		
    # Rotate drawn points about Y_AXIS to be perpendicular to picked face normal
    @pts0.each_index {|i| @pts0[i].transform! @tf5 }
