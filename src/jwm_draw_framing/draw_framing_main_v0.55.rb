...
## Attempting to:
## Implement keyboard choice of axis (using standard SU keys) to toggle axis constraint on/off:
##   red = Right Arrow, blue = Up or Down Arrow, green = Left Arrow
## and constrain mouse moves to orientation around selected axix
## Implement keyboard choice of orientation of cross section of timber, 
##   using Tab key to cycle through options
...
module JWM
	...
	puts "******************************"
	puts "Class DrawFraming v0.55 loaded"

		@@size_index = 1
		@@nom_size = [0.0.to_l,0.0.to_l] # Declare array to hold current nominal size as lengths
		@@custom_size = ["Custom default",0.0.to_l,0.0.to_l]# Declare array to hold last defined custom size
...
## 	Initialize is run each time the tool is selected
  ...
  # Axis_lock 0, 1, 2 for red, green, blue; false = unlocked; true = lock
    ...
    @axis_lock[0] = true # Default (red) axis for long dimension of timber
    @axis_lock[1] = false
    @axis_lock[2] = false
...
# puts @@df_tool_id.to_s
  ...
  when VK_RIGHT # Right arrow key pressed: toggle red axis lock on/off
    if @axis_lock[0] then # Axis lock was on: turn it off
      @axis_lock[0] = false
    else
      @axis_lock[0] = true # turn red axis lock on, and turn other axes off
      @axis_lock[1] = false
      @axis_lock[2] = false
    end
  when VK_LEFT # Left arrow key pressed: toggle green axis lock on/off
    if @axis_lock[1] then # Axis lock was on: turn it off
      @axis_lock[1] = false
    else
      @axis_lock[1] = true # turn green axis lock on, and turn other axes off
      @axis_lock[0] = false
      @axis_lock[2] = false
    end
  when VK_UP # Up  arrow key pressed: toggle blue axis lock on/off
    if @axis_lock[2] then # Axis lock was on: turn it off
      @axis_lock[2] = false
    else
      @axis_lock[2] = true # turn blue axis lock on, and turn other axes off
      @axis_lock[0] = false
      @axis_lock[1] = false
    end
